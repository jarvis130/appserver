...
class Order extends BaseModel
{
    ...
    public static function review(array $attributes, $items)
    {
        ...
        if ($model = self::where(['user_id' => $uid, 'order_id' => $order])->first()) {
            ...
            $comment_check_row = ShopConfig::where('code', 'comment_check')->first();
            $comment_status = $comment_check_row->value==1?0:1;
            foreach ($items as $key => $value) {
                // 提交评价
                if ($comment = Comment::toCreate($uid, $value, $is_anonymous,$comment_status)) {
    ...
    {
        return ($this->attributes['order_amount'] + $this->attributes['money_paid'] + $this->attributes['surplus']);
    }
